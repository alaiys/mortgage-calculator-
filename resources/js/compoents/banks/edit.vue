...
const getBank = async () => {
    let response = await axios.get(`/api/bank/${props.id}`)
    form.value = response.data.bank
}
...
const updateBank = () => {
    ...
    axios.post(`/api/bank/${form.value.id}`, formData)
        .then((response) => {
            form.value.name=''
